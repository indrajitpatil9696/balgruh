...
function fide_all_question(){
	...
	for(var i=0; i < noq; i++){
		...
		var did="#q"+i;
		$(did).removeClass('active_question');
...
function show_next_question(){
	...
	$(did).addClass('active_question');
	$(did).find('input:checked').click();
	}
	// hide show next back btn
	if(qn >= 1){
	$('#backbtn').css('visibility','visible');
...
function update_check(sq_version){
 ...
 jQuery(document).ready(function(){
	 $('#navbar').click(function(){
		 var active = $('#navbar').hasClass('change');
		 $('#navbar').toggleClass('change');
		 $('#question-sidebar').toggleClass('open');
	 });
	 
	 $(document).on('change click','.ui-radio input',function(){
		 console.log( $(this).parents('div.op_style').siblings('.op_style').find('p'));
		 $(this).parents('div.op_style').addClass('selected').siblings('.op_style').removeClass('selected').find('p').css('font-weight','normal');
		 if ($(this).is(':checked')) {
			 $(this).parent().parent().next().children().css('font-weight','bold');
		 }
	 });
	 
	 $('div.op_style input:checked').click();
 });
